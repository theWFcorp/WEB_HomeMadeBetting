...
const DYN_MAXMULT = 2;    // потолок: динамический кэф не выше 2× от стартового (без разлётов)
const DYN_MAXABS = 10;    // и абсолютный потолок динамического кэфа
const DYN_MIN = 1.05;     // пол динамического кэфа
...
function dynamicOdds(market) {
  ...
  market.selections.forEach(sel => {
    ...
    let o = (1 - comm) / p;
    o = Math.min(o, sel.odd * DYN_MAXMULT, DYN_MAXABS);   // потолок: 2× от стартового и не выше 10
    out[sel.id] = Math.max(DYN_MIN, Math.round(o * 100) / 100);
  });
  return out;
}
